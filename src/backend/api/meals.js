...
function checkTableData(table) {
  try {
    ...
    if ("when" in validTableData) {
      const whenFormatted = new Date(validTableData.when);
      if (!Date.parse(whenFormatted)) {
        throw new Error();
      }
    }
    if ("max_reservations" in validTableData) {
      if (isNaN(Number(validTableData.max_reservations))) {
        throw new Error();
      }
    }
    if ("price" in validTableData) {
      if (isNaN(Number(validTableData.price))) {
        throw new Error();
      }
    }
      ...
      if (maxPrice) {
        if (isNaN(Number(maxPrice))) {
          return response.send(
            400,
            "Please provide valid number for maxPrice."
          );
        } else {
          filteredMeals = await meals.where("price", "<", maxPrice);
        }
      }
      if (availableReservations) { // this query doesn't work, I've tryed different ways but still don't know how to fix it
...
router.post("/", async (request, response) => {
  try {
    ...
    const insertMeal = await knex("meals").insert(validTableData);
